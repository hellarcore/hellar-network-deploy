 ...
 hp_masternodes_arm = [
  ...
  wallets = [
    for n in range(length(aws_instance.hellard_wallet)) : templatefile(
      "${path.module}/templates/services/node.tpl",
      {
        name = "hellard-wallet-${n + 1}"
        external_services = replace(
          chomp(join("", [local.service_ssh])),
          "{{ip}}",
          element(aws_instance.hellard_wallet.*.public_ip, n),
        )
        internal_services = replace(
          chomp(
            join(
