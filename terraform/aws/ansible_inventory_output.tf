locals {
  ...
  wallet_node_hosts = [
    for n in range(length(aws_instance.hellard_wallet)) : templatefile(
      "${path.module}/templates/inventory/hostname.tpl",
      {
        index      = n + 1
        name       = element(aws_instance.hellard_wallet.*.tags.Hostname, n)
        public_ip  = element(aws_instance.hellard_wallet.*.public_ip, n)
        private_ip = element(aws_instance.hellard_wallet.*.private_ip, n)
      }
    )
  ]
  ...
  ansible_inventory = templatefile(
    "${path.module}/templates/inventory/ansible_inventory.tpl",
    {
      ...
      wallet_node_hosts   = join("\n", concat(aws_instance.hellard_wallet.*.tags.Hostname))
